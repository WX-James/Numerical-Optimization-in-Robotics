...
namespace lbfgs
{
                                        ...
                                        const lbfgs_parameter_t &param)
    {
        ...
        int search_number = 0;
        double up_bound = stpmax;
        double low_bound = 0.0;

        double dg_init = gp.dot(s);
        bool touched = false;

        if(dg_init > 0.0)
        {
            //std::cout <<"JJJJJJJJJJJJL " << std::endl;
        ...
        double f_init = f;
        double stp_init = stp;
        double dg_armijo = param.f_dec_coeff * dg_init;
        double dg_wolfe = param.s_curv_coeff * dg_init;
        ...
        while(true)
        {
            ...
            search_number ++;
            if(f > f_init + stp * dg_armijo)
            {
                //std::cout <<"armijo fail"<< std::endl;
                up_bound = stp;

            }
            else if(g.dot(s) < dg_wolfe)
            {
                //std::cout <<"wolfe_fail"<< std::endl;
                low_bound = stp;
            }
            else
            {
            ...
            if (search_number >= param.max_linesearch )
            {
                std::cout <<"[L-BFGS:] " << "LBFGSERR_INVALID_MAXLINESEARCH" << std::endl;
                std::cout << "stp: " << stp << std::endl;
                std::cout << "g_final:" << g.transpose() << std::endl;
                return LBFGSERR_INVALID_MAXLINESEARCH;
                
            }

            if ((up_bound < stpmax) &&  (up_bound-low_bound) < param.machine_prec * up_bound)
            {
                ...
                */
                if (0 < param.past)
                {
                    ...
                    if (param.past <= k)
                    {
                        ...
                        //std::cout << "stopping rate: " << rate << std::endl;
                        //std::cout << "param.delta: " << param.delta << std::endl;
